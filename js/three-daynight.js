...
THREEx.DayNight.SunLight = function() {
    ...
    this.update = function(sunAngle) {
        light.position.x = 0;
        light.position.y = (Math.sin(sunAngle) * window.size / Math.PI);
        light.position.z = Math.cos(sunAngle) * window.size / Math.PI;
        //console.log('Phase', THREEx.DayNight.currentPhase(sunAngle))
        var phase = THREEx.DayNight.currentPhase(sunAngle);
        world.starField.visible = true;
        if (phase === 'day') {
            light.intensity = .005;
            world.starField.visible = false;
            light.color.setRGB(255, Math.floor(Math.sin(sunAngle) * 200) + 55, Math.floor(Math.sin(sunAngle) * 200) + 55);
        } else if (phase === 'twilight') {
            light.intensity = 1;
            light.color.setRGB((255 - Math.floor(Math.sin(sunAngle) * 510 * -1)), (55 - Math.floor(Math.sin(sunAngle) * 110 * -1)), 0);
        } else {
            light.intensity = 0;
        }
    }
...
THREEx.DayNight.SunSphere = function() {
    ...
    this.update = function(sunAngle) {
        mesh.position.x = 0;
        mesh.position.y = Math.sin(sunAngle) * window.size / Math.PI;
        mesh.position.z = Math.cos(sunAngle) * window.size / Math.PI;
        var phase = THREEx.DayNight.currentPhase(sunAngle)
        if (phase === 'day') {
